...
def monitor_browser_window():
    ...
    try:
        ...
        has_psutil = False
        console.print(
            "[yellow]Note: psutil not available, using alternative browser monitoring[/yellow]"
        )
                ...
                try:
                    ...
                    if os.name == "posix":  # Unix/Linux/Mac
                        result = subprocess.run(
                            ["pgrep", "-f", "chrome"], capture_output=True, text=True
                        )
...
def get_llm_provider():
    ...
    if not available_providers:
        console.print(
            "[red]No LLM API keys found. Please set at least one of: "
            "OPENAI_API_KEY, ANTHROPIC_API_KEY, or DEEPSEEK_API_KEY[/red]"
        )
        return None
...
💡 Find this tool helpful? Support the project: https://buymeacoffee.com/seanpavlak
        ...
        discussion_parser = subparsers.add_parser(
            "discussion", help="Scrape discussions and generate AI responses"
        )
        ...
        announcement_parser = subparsers.add_parser(
            "announcement", help="Schedule course announcements"
        )
        announcement_parser.add_argument(
