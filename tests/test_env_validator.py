...
class TestSettings:
    ...
    def test_get_available_providers_all(self, tmp_path, monkeypatch):
        ...
        settings = Settings(
            CANVAS_OP_ITEM="Instructure - CHCP",
    ...
    def test_get_available_providers_single(self, tmp_path, monkeypatch):
        ...
        settings = Settings(
            CANVAS_OP_ITEM="Instructure - CHCP", OPENAI_API_KEY="sk-test"
        )
        providers = settings.get_available_providers()
    ...
    def test_validate_llm_keys_failure(self, tmp_path, monkeypatch):
        ...
        settings = Settings(CANVAS_OP_ITEM="Instructure - CHCP")
        with pytest.raises(ValueError, match="At least one LLM API key"):
            settings.validate_llm_keys()

    def test_requires_op_item(self, tmp_path, monkeypatch):
        """CANVAS_OP_ITEM is required"""
        monkeypatch.chdir(tmp_path)
        with pytest.raises(Exception):
            Settings(CANVAS_OP_ITEM="  ")
